...
const Hero = ({ go }) => (
  <section className="relative overflow-hidden" style={{ background: C.porcelain }}>
    <div className="max-w-6xl mx-auto px-6 pt-14 pb-20 grid md:grid-cols-2 gap-12 items-center">
      ...
      <div className="relative fade-in hidden md:block" style={{ animationDelay: ".1s" }}>
        <div className="absolute -inset-4 arch-full" style={{ border: `1px solid ${C.gold}66`, transform: "translate(14px, 14px)" }} />
        <ArchImage src={HERO_IMAGE} alt="Elegant event reception styled by PNP" ratio="120%" full />
        <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
          <span className="f-display" style={{ color: "#fff", fontSize: 22, fontStyle: "italic", textShadow: "0 2px 12px rgba(0,0,0,.5)" }}>Posh & Polished Events</span>
        </div>
      </div>
    </div>
  </section>
...
const ProductCard = ({ p, onOpen, onAdd }) => (
  <div className="fade-in">
    <button className="w-full text-left card-lift" style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }} onClick={() => onOpen(p)}>
